...
- Create and edit workspace-level assets under `.pocketcode/`, especially flat `*.agent.md`, `*.agent.yaml`, `*.tool.py`, `*.tool.md`, `*.prompt.md`, and `.pocketcode/skills/`.
- Handle self-contained `*.md` flows, adjacent `*.tool.py`, `*.prompt.md`, flat `*.agent.yaml`, `*.agent.md`, `SKILL.md`, and related documentation.
...
- Prefer qualified names like `namespace::flow` or `namespace::tool` when referencing cross-namespace resources.
