"""
Koder agent PocketFlow factory.

The Koder agent specialises in coding tasks: writing code, creating files,
...
class KoderThinkNode(Node):
    """Think/exec node for the Koder agent."""
    ...
    def post(self, shared: Dict[str, Any], prep_res: str, exec_res: str) -> str:
        ...
        if llm_router is not None:
            return "llm_delegate"
        logger.debug("KoderThinkNode: no _llm_router in shared; skipping LLM call.")
        shared.setdefault("results", {})["koder"] = {
            "status": "pending_llm",
            "request": exec_res,
        }
        return "continue"
...
def create_flow() -> Flow:
    """Return a Koder agent Flow."""
    think = KoderThinkNode()
