...
- `flows/normalize.md`: StackVM-backed flow description with explicit VM module wiring
- `vm/common.vm`: declared helper module exporting the YAML request payload and parsed message extraction words under `common.*`
- `vm/router.vm`: user-authored macros plus the caller logic that uses qualified `common.*` helper words and macros

The example demonstrates:

- using an explicit helper module instead of repeating a YAML request literal
