...
- `vm/router.vm`: caller script that normalizes data with `parallel-map` and finalizes from the delegate return payload
- `vm/delegate.vm`: delegate script that uses `prompt-interaction` and returns a final answer to the caller

The example demonstrates:

- tool-first normalization of all payload item titles in a VM caller
...
- finalizing from `last_delegated_result` after a StackVM delegate returns