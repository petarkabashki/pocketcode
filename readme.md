...
- top header `Agent` and `LLM` chips open their respective searchable popup pickers directly

Interactive workspace views:

- `Chat`: conversation and command entry
- `Control`: runtime/session selectors and toggles for workspace mode, theme, active flow, active agent profile, LLM, and session confirmation
- `Control`: includes a popup asset picker for selecting active flow/profile/LLM/theme/mode/session state from searchable lists with arrow-key navigation
- top header `Agent` and `LLM` chips mirror the current selection and open the matching picker without leaving the current view
...
- the first row is always visible and shows `Runtime flow`, `Flow`, `Agent`, and `LLM`
...
- it does not repeat the active flow/agent state already shown in the status bar
