...
- `vm program`: the preferred executable authoring unit for new work. In practice this is a self-contained Markdown agent file with StackVM plus optional sibling `*.tool.py` / `*.prompt.md` files or explicit relative `tool_files`.
...
- `workspace namespace`: a runtime namespace loaded either from a plain folder such as `.github/` or from a flat namespace-pack root such as `.pocketcode/`, where filenames carry the namespace prefix like `coder.coder.md`, `coder.coder.agent.md`, or `coder.git.tool.py`.
...
- `modes_and_skills.md`: current skill behavior
...
7. Read `modes_and_skills.md` when working on skills.
