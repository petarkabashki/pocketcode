...
StackVM-backed flows execute against the same shared-store contract used by handwritten PocketFlow flows, including `_tool_runtime`, `pending_handoff_agent`, `final_answer`, `question_to_ask`, `results`, and other runtime-managed keys. The current execution boundary is now split into two layers:

- `pocketcode/core/agent_stack_vm.py`: the core VM plus StackVM runtime words
- `pocketcode/core/stackvm_host.py`: the current host adapter layer that interprets VM host words for the PocketCoder runtime

The PocketCoder adapter still writes the familiar shared-store compatibility keys, but the VM no longer hardcodes those writes directly inside each host word. VM host words now also record:
...
The shared typed effect schema now lives in `pocketcode/core/runtime_effects.py`. StackVM host words and the broader agent decision path both use that module's constructors and transition-derivation helpers so runtime actions such as tool calls, answers, questions, handoffs, and explicit transitions share one canonical shape. The current PocketCoder-specific adapter implementation lives in `pocketcode/core/stackvm_host.py` as `PocketCoderStackVmHostAdapter`, which is now the main seam for future non-PocketCoder host adapters.
...
The `/stackvm run` and `/stackvm debug` CLI commands now use two script-execution paths. Registered StackVM flows still execute directly against their loaded `FlowDefinition` through the normal agent runtime. Standalone scripts under `<primary_resource_root>/vm/` now prefer a lightweight standalone VM path backed by `StandaloneStackVmHostAdapter` in `pocketcode/core/stackvm_host.py` when their static host-surface summary stays within the portable subset. That portable subset now includes direct `llm-call`, callback-backed `prompt-user` and `prompt-interaction`, synchronous `tool-call`, `ask-user`, and portable runtime-state readers such as `last-tool-result`. Scripts that use PocketCoder-only orchestration words such as `tool-request`, `handoff`, or session-oriented readers such as `active-session-transcript` still fall back to a temporary synthetic `FlowDefinition` so they continue to use the full PocketCoder runtime loop.

The same portable runtime is now exposed programmatically in `pocketcode/core/stackvm_driver.py`. `compile_stackvm_program(...)` produces the expanded AST plus static analysis payload for an arbitrary source string, and `run_compiled_stackvm_program(...)` executes that compiled program against either explicit hooks or one `StackVmStandaloneHostConfig` object for tool runtime, LLM router, interaction handling, runtime event observation, tool definitions, system prompt, and auto-confirm policy. `build_stackvm_standalone_host_config(...)` is the preferred constructor because it can derive tool definitions from the supplied tool runtime automatically. The same module now also exposes `compile_stackvm_program_target(...)` and `run_stackvm_program_target(...)`, which accept the normal StackVM loader fields such as `vm_source`, `vm_module`, `vm_modules`, `vm_file`, and `vm_files` so embedders can reuse the same module/file resolution semantics as VM flows and CLI scripts.

For repeated invocation, the same driver surface now also exposes `StackVmStandaloneRuntime`, `create_stackvm_standalone_runtime(...)`, and `create_stackvm_standalone_runtime_target(...)`. That reusable harness keeps one compiled program, one default standalone host config, one workspace root, one agent name, and one shared-store seed, then runs repeated requests through the existing standalone driver path with optional per-run overrides.

When a caller wants stateful reuse instead of fresh seeded runs, the same module now also exposes `StackVmStandaloneSession`, `create_stackvm_standalone_session(...)`, and `create_stackvm_standalone_session_target(...)`. A standalone session persists a curated shared-store snapshot plus a simple transcript across runs. The current transcript is injected back into each run as `standalone_transcript` and `standalone_transcript_text`, which gives embedded programs a portable conversational context surface without depending on the PocketCoder-only saved-session manager or `active-session-transcript` host words.

That same session layer now also has an explicit snapshot boundary. `StackVmStandaloneSession.snapshot()` returns the current portable session payload, `restore(...)` reloads that payload into an existing session, and the driver-level `restore_stackvm_standalone_session(...)` and `restore_stackvm_standalone_session_target(...)` helpers recreate a session from a saved snapshot plus either an existing runtime or one fresh target definition. `PocketCodeEngine` continues to reuse the stateless driver path for its standalone script execution instead of maintaining a second embedded-only implementation.

StackVM module resolution for VM-backed flows now also includes `workspace_root` in the search roots. That gives flows a stable shared package location under workspace-root `vm/stdlib/` in addition to namespace-local `vm/` helpers. When `vm/stdlib/stdlib.yaml` is present, the loader also resolves manifest-backed stdlib aliases like `stdlib.io`, `stdlib.config`, `stdlib.normalize`, `stdlib.prompt`, and `stdlib.returns` directly.
